...
import { Ambulance, BrainCircuit, TrafficCone, Building2 as Hospital, Map as MapIcon, Database, Server, Smartphone, Zap, Activity, Shield } from 'lucide-react';
...
function LoadingScreen({ progress }) {
  return (
    <div style={{
      ...
      background: '#0a0e17', display: 'flex', flexDirection: 'column',
      ...
      <div style={{ width: '240px', height: '4px', borderRadius: '2px', background: 'rgba(255,255,255,0.1)', overflow: 'hidden' }}>
        <div style={{
          width: `${progress}%`, height: '100%', borderRadius: '2px',
          background: 'linear-gradient(90deg, #10b981, #06b6d4)',
          transition: 'width 0.2s', boxShadow: '0 0 10px rgba(16,185,129,0.4)'
        }} />
      </div>
...
// NAVBAR
// ═══════════════════════════════
function Navbar({ scrollPercent, onOpenHospital, onOpenPolice }) {
  return (
    <nav className="navbar" style={{ opacity: scrollPercent > 0.02 ? 1 : 0.8 }}>
      <div className="navbar-inner">
        ...
        <div className="nav-links">
          ...
          <button onClick={onOpenHospital} style={{
            background: 'rgba(139,92,246,0.12)', border: '1px solid rgba(139,92,246,0.25)',
            color: '#a78bfa', borderRadius: '8px', padding: '6px 14px', cursor: 'pointer',
            fontFamily: 'Inter', fontWeight: 600, fontSize: '0.8rem', transition: 'all 0.2s',
          }}
          ...
          <button onClick={onOpenPolice} style={{
            background: 'rgba(99,102,241,0.12)', border: '1px solid rgba(99,102,241,0.25)',
            color: '#818cf8', borderRadius: '8px', padding: '6px 14px', cursor: 'pointer',
            fontFamily: 'Inter', fontWeight: 600, fontSize: '0.8rem', transition: 'all 0.2s',
          }}
    ...
    <section className="scroll-section bg-stars" style={{ minHeight: '110vh' }}>
      ...
      <div style={{
        ...
        <div className="animate-float-slow" style={{
          ...
          color: '#10b981', fontSize: '0.875rem', fontWeight: 600, backdropFilter: 'blur(8px)'
        }}>
      ...
      {scrollY < 300 && (
        <div style={{
          ...
          <div style={{ width: '24px', height: '40px', borderRadius: '12px', border: '1.5px solid rgba(255,255,255,0.3)', position: 'relative' }}>
            <div style={{
              width: '4px', height: '8px', borderRadius: '2px', background: '#10b981',
...
function FrameAnimationSection({ images, loaded }) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      setIsInView(inView);
      if (inView) {
        const progress = Math.min(1, Math.max(0, -rect.top / Math.max(1, sectionRef.current.offsetHeight)));
        setDisplayFrame(Math.min(TOTAL_FRAMES - 1, Math.floor(progress * (TOTAL_FRAMES - 1))));
      }
    };
    window.addEventListener('scroll', handleScroll, { passive: true });
    handleScroll();
  ...
  return (
    <>
      <section id="animation" ref={sectionRef} style={{ height: '400vh', position: 'relative' }} />
      {loaded && isInView && (
        <div style={{
          position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh',
          zIndex: 50, background: 'var(--color-bg-dark)',
          display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none',
        }}>
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse at center, transparent 40%, var(--color-bg-dark) 100%)', zIndex: 2 }} />
          <img src={getFrameSrc(displayFrame)} alt={`Frame ${displayFrame + 1}`} style={{
            maxWidth: '90%', maxHeight: '85vh', borderRadius: '16px', objectFit: 'contain', zIndex: 1,
            boxShadow: '0 0 60px rgba(16,185,129,0.1), 0 0 120px rgba(6,182,212,0.05), 0 25px 50px rgba(0,0,0,0.5)',
          }} />
        </div>
      )}
    </>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', width: '100%' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '60px', alignItems: 'center' }}>
          <div className={`section-reveal ${isVisible ? 'visible' : ''}`}>
            <h2 style={{ fontFamily: 'Outfit', fontSize: 'clamp(2rem, 4vw, 2.75rem)', fontWeight: 800, marginBottom: '24px', lineHeight: 1.15 }}>
              The Challenge in Cities <br />
              <span className="gradient-text-red">Like Bangalore</span>
            </h2>
            <p style={{ fontSize: '1.1rem', color: 'rgba(226,232,240,0.6)', lineHeight: 1.8, marginBottom: '32px' }}>
              Traffic congestion in metropolitan areas is costing lives. Ambulances routinely face delays of 15–30 minutes, drastically reducing the "golden hour" survival rate.
            </p>
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {[
                ...
                <li key={i} className={`section-reveal stagger-${i + 1} ${isVisible ? 'visible' : ''}`} style={{ display: 'flex', alignItems: 'center', gap: '12px', color: 'rgba(226,232,240,0.7)', fontSize: '1rem' }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', flexShrink: 0, background: '#ef4444', boxShadow: '0 0 8px rgba(239,68,68,0.5)' }} />
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div className={`section-reveal stagger-2 ${isVisible ? 'visible' : ''}`}>
            <div className="glass-panel" style={{ padding: '40px', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(239,68,68,0.05), transparent)', pointerEvents: 'none' }} />
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px' }}>
                {[100, 75, 88, 100, 65].map((w, i) => (
                  <div key={i} className="congestion-line" style={{ width: `${w}%`, animationDelay: `${i * 0.3}s` }} />
                ))}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', padding: '16px 24px', borderRadius: '12px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', backdropFilter: 'blur(8px)' }}>
                <span style={{ width: 10, height: 10, borderRadius: '50%', background: '#ef4444', animation: 'pulse-glow 1.5s ease-in-out infinite' }} />
...
function CorridorSection() {
  ...
  const steps = [
    ...
    { icon: Hospital, label: 'Hospital', sub: 'Approves Request' },
    { icon: Shield, label: 'Police', sub: 'Confirms Escort' },
    { icon: BrainCircuit, label: 'AI Routing', sub: 'Calculates Path' },
    { icon: TrafficCone, label: 'Traffic', sub: 'Signals Green' },
    ...
    <section id="corridor" ref={(el) => { ref.current = el; sectionRef.current = el; }} className="scroll-section bg-grid" style={{ padding: '120px 0', minHeight: 'auto' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', width: '100%' }}>
        <div className={`section-reveal ${isVisible ? 'visible' : ''}`} style={{ textAlign: 'center', marginBottom: '64px' }}>
          <h2 style={{ fontFamily: 'Outfit', fontSize: 'clamp(2rem, 4vw, 2.75rem)', fontWeight: 800, marginBottom: '16px' }}>
          ...
          <p style={{ color: 'rgba(226,232,240,0.6)', fontSize: '1.1rem', maxWidth: '600px', margin: '0 auto' }}>
            3-layer approval chain: Hospital → Police → AI routing → Signal preemption.
          </p>
        </div>
        ...
        <div className={`section-reveal stagger-2 ${isVisible ? 'visible' : ''}`} style={{ marginBottom: '80px' }}>
          <div className="glass-panel" style={{ padding: '40px', overflow: 'hidden' }}>
            <div style={{ position: 'relative', height: '80px', marginBottom: '24px' }}>
              <div style={{ position: 'absolute', top: '50%', left: '0', right: '0', height: '8px', transform: 'translateY(-50%)', background: 'rgba(255,255,255,0.06)', borderRadius: '4px' }} />
              <div style={{ position: 'absolute', top: '50%', left: '0', width: `${Math.min(100, greenCount * 22)}%`, height: '8px', transform: 'translateY(-50%)', borderRadius: '4px', transition: 'width 0.6s ease' }} className="corridor-line" />
              <div style={{
                position: 'absolute', top: '50%', left: `${Math.min(88, greenCount * 20)}%`,
                transform: 'translate(-50%, -50%)', transition: 'left 0.6s ease',
                background: 'white', borderRadius: '8px', padding: '8px',
                boxShadow: '0 0 20px rgba(255,255,255,0.3)', zIndex: 10
              }}>
                <Ambulance style={{ width: 20, height: 20, color: '#ef4444' }} />
              </div>
              {[20, 40, 60, 80].map((pos, i) => (
                <div key={i} style={{ position: 'absolute', top: '-4px', left: `${pos}%`, transform: 'translateX(-50%)' }}>
                  ...
                  {i < greenCount && (
                    <div style={{ marginTop: '4px', fontSize: '0.6rem', color: '#10b981', textAlign: 'center', fontWeight: 600, whiteSpace: 'nowrap' }}>
                      AI Preempted
                    </div>
                  )}
                </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 20px', borderRadius: '12px', background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.15)' }}>
        ...
        <div className={`section-reveal stagger-3 ${isVisible ? 'visible' : ''}`}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', flexWrap: 'wrap', position: 'relative' }}>
            <div style={{ position: 'absolute', top: '36px', left: '40px', right: '40px', height: '2px', background: 'linear-gradient(90deg, rgba(16,185,129,0.2), rgba(6,182,212,0.3), rgba(16,185,129,0.2))', zIndex: 0 }} />
            {steps.map((step, i) => (
              <div key={i} className="flow-step" style={{ zIndex: 1, flex: '1 1 100px' }}>
                <div className="flow-icon-box">
                  <step.icon style={{ width: 24, height: 24 }} />
                </div>
                <h3 style={{ fontFamily: 'Outfit', fontWeight: 700, fontSize: '0.875rem' }}>{step.label}</h3>
                <span style={{ fontSize: '0.7rem', color: 'rgba(226,232,240,0.5)' }}>{step.sub}</span>
              </div>
            ))}
          </div>
        </div>
...
function ImpactSection() {
  ...
  const techStack = [
    { icon: Server, name: 'Node.js' }, { icon: Smartphone, name: 'React' },
    { icon: Database, name: 'MongoDB' }, { icon: Zap, name: 'WebSockets' },
    { icon: MapIcon, name: 'Google Maps' }, { icon: BrainCircuit, name: 'TensorFlow' }
  ];
  return (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '24px', marginBottom: '64px' }}>
          {stats.map((stat, i) => (
            <div key={i} className={`stat-card section-reveal stagger-${i + 1} ${isVisible ? 'visible' : ''}`}>
              ...
              <div style={{ fontFamily: 'Outfit', fontWeight: 700, fontSize: '1.1rem', marginBottom: '8px' }}>{stat.label}</div>
              <p style={{ fontSize: '0.85rem', color: 'rgba(226,232,240,0.5)', lineHeight: 1.6 }}>{stat.desc}</p>
            </div>
          ))}
        </div>
        <div className={`section-reveal stagger-4 ${isVisible ? 'visible' : ''}`}>
          <div className="glass-panel" style={{ padding: '32px', display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '40px', alignItems: 'center' }}>
            {techStack.map((tech, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'rgba(226,232,240,0.5)', transition: 'color 0.3s', cursor: 'default' }}
...
// ═══════════════════════════════
// PORTALS SECTION (5 portals)
// ═══════════════════════════════
function PortalsSection({ onOpenSimulation, onOpenHospital, onOpenPolice }) {
  const [ref, isVisible] = useInView(0.1);
  ...
  const portals = [
    {
      title: 'User Portal',
      icon: Smartphone,
      desc: 'Request emergency ambulance, select hospital, track real-time arrival. Your request triggers the full approval chain.',
      gradient: 'linear-gradient(135deg, #3b82f6, #6366f1)',
      action: onOpenSimulation,
      label: 'Request Ambulance',
    },
    {
      title: 'Ambulance Portal',
      icon: Ambulance,
      desc: 'Turn-by-turn AI-synced navigation, auto signal preemption triggers, and hospital communication.',
    ...
    },
    {
      ...
      icon: TrafficCone,
      desc: 'City-wide dashboard to monitor active corridors, override signals, and view live analytics.',
    ...
    },
    {
      ...
      icon: Hospital,
      desc: 'Receive incoming ambulance requests in real time. Your approval is mandatory before dispatch begins.',
      gradient: 'linear-gradient(135deg, #8b5cf6, #ec4899)',
      action: onOpenHospital,
      label: 'Open Hospital Portal',
    },
    {
      title: 'Police Portal',
      icon: Shield,
      desc: 'Confirm police escort after hospital approval. Your confirmation triggers ambulance dispatch and corridor activation.',
      gradient: 'linear-gradient(135deg, #6366f1, #3b82f6)',
      action: onOpenPolice,
      label: 'Open Police Portal',
    },
  ];
    ...
    <section id="portals" ref={ref} className="scroll-section" style={{ padding: '120px 0', minHeight: 'auto' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', width: '100%' }}>
        <div className={`section-reveal ${isVisible ? 'visible' : ''}`} style={{ textAlign: 'center', marginBottom: '64px' }}>
          ...
          <p style={{ color: 'rgba(226,232,240,0.6)', fontSize: '1.1rem', maxWidth: '600px', margin: '0 auto' }}>
            Five dedicated portals. One seamless emergency response network.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
          {portals.map((portal, i) => (
            <div
              key={i}
              className={`portal-card section-reveal stagger-${(i % 4) + 1} ${isVisible ? 'visible' : ''}`}
            ...
            >
              ...
              <p style={{ fontSize: '0.9rem', color: 'rgba(226,232,240,0.5)', lineHeight: 1.7, marginBottom: '20px', flexGrow: 1 }}>
                {portal.desc}
              </p>
              <button
                onClick={portal.action}
                disabled={!portal.action}
                style={{
                  fontSize: '0.85rem', fontWeight: 700,
                  color: portal.action ? '#10b981' : 'rgba(100,116,139,0.4)',
                  cursor: portal.action ? 'pointer' : 'default',
                  display: 'inline-flex', alignItems: 'center', gap: '4px',
                  background: 'none', border: 'none', padding: 0,
                  borderBottom: portal.action ? '1px solid rgba(16,185,129,0.3)' : '1px solid transparent',
                  paddingBottom: '2px', transition: 'color 0.2s, border-color 0.2s',
                }}
                onMouseOver={e => { if (portal.action) e.currentTarget.style.color = '#34d399'; }}
                onMouseOut={e => { if (portal.action) e.currentTarget.style.color = '#10b981'; }}
              >
                {portal.label} {portal.action ? '→' : ''}
              </button>
            </div>
          ))}
        </div>
...
function FinaleSection({ images, loaded }) {
  ...
  return (
    <section className="scroll-section" style={{ position: 'relative', overflow: 'hidden' }}>
      <canvas ref={canvasRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: 0.25, filter: 'blur(4px)' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, var(--color-bg-dark) 20%, transparent 80%)', pointerEvents: 'none' }} />
      <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', maxWidth: '700px', padding: '0 24px' }}>
        <div className="text-glow-strong" style={{ fontFamily: 'Outfit', fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, marginBottom: '20px', lineHeight: 1.15 }}>
          Every Second <span className="gradient-text-emerald">Counts</span>
        </div>
        <p style={{ color: 'rgba(226,232,240,0.6)', fontSize: '1.1rem', lineHeight: 1.7, marginBottom: '32px' }}>
...
function Footer() {
  return (
    <footer style={{ padding: '40px 24px', textAlign: 'center', borderTop: '1px solid rgba(255,255,255,0.05)', color: 'rgba(226,232,240,0.3)', fontSize: '0.85rem', fontFamily: 'Inter' }}>
...
export default function App() {
  ...
  // Shared request state (passed across portals) - synced via MongoDB Backend
    ...
    const fetchActiveRequest = async () => {
      try {
        ...
        if (data.success && data.data) {
          ...
          setHospitalApproved(req.status !== 'WAITING_HOSPITAL');
          setPoliceApproved(req.status === 'ASSIGNED' || req.status === 'TRACKING' || req.status === 'ARRIVED');
        } else {
